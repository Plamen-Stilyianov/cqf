...
import static FinApps.PresentValue.pVonecash;
...
public class Spread extends NewtonRaphson {
     ...
     * <p/>
     * provides the static spread from the corporate bond price. The amount by which each spot needs to be adjusted
    ...
    /**
     * Calculates the yield spread for YTM of corretly priced risk free and arbitrary priced corporatecorporate
     */
    public double spreadrateS(double[] spotrates, double priceval, double couponrate, double maturity, double yieldapprox) {
        ...
        spotapprox = (((spots[0] + spots[(spots.length - 1)]) / 2.0) / 100.0);//first guess
        ...
        curveyield = c.yieldEstimate(facevalue, 6.0, coupon, price, maturity, spotapprox);
        price = priceval;
        Tyield t = new Tyield();
        baseyield = t.yieldEstimate(facevalue, 6.0, coupon, price, maturity, periodyield);
        return (abs(baseyield - curveyield));// returns annualised rates
    }

    /**
     * Assumes annual coupon and annual yield with years to maturity as input parameters
    ...
    public double[] spreadrateS(double[] spotrates, double prices[], double couponrate, double maturity, double yieldapprox) {
        ...
        curveyield = t.yieldEstimate(facevalue, 6.0, coupon, price, maturity, (curvest / 100.0));

        for (double p : prices) {
            Tyield yld = new Tyield();
            price = p;
            double y = yld.yieldEstimate(facevalue, 6.0, coupon, price, maturity, (yieldapprox / 100.0));
    ...
    public double[] spreadsT(double[] spotrates, double prices[], double couponrate, double maturity, double yieldapprox) {
        spots = spotrates;
        coupon = couponrate;
        int index = 0;
        double spreads[] = new double[prices.length];
    ...
    private double spotPvperiod(double[] periodspot, double coupon) {
        ...
        par = (100.0 * exp(-(double) size * log(1.0 + (periodspot[(periodspot.length - 1)] / 100.0))));
        return (pv + par);
    }

    /**
     * Method to compute the PV of an array of annual spots and annual coupon with given annual frequency of compounding
     * *
     */
    private double spotPvannual(double[] periodspot, double coupon) {
        ...
        if (terms > 1.0) {
            ...
            par = (100.0 * exp(-(double) size * log(1.0 + (periodspotadj[(periodspotadj.length - 1)] / 100.0))));
            ...
            double couponadjust = (coupon / terms); //from an annual coupon to the period rate
            pv = pVonecash(periodspot, couponadjust);
            par = (100.0 * exp(-(double) periodspot.length * log(1.0 + (periodspot[(periodspot.length - 1)] / 100.0))));
    ...
    /**
     * Assumes period spot and the yield approximation are period rates with period percentages
     * Assumes flat rate spot for entire maturity period
     * assumes the coupon is an annual coupon percent of par rate.
     */
    ...
    public double spreadrate(double periodspot, double priceval, double coupon, double maturity, double yieldapprox) {
        ...
        return (abs(baseyield - periodspot) * terms * 100.0);//returns annualised spread
    }
    ...
    public double[] creditS(double[] riskless, double[] risky) {
        ...
        for (int i = 1; i < size; i++) {
            fdefault[i] = (1.0 - (exp(-log(1.0 - pdefault[i - 1]))) * (risky[i] / riskless[i]));
