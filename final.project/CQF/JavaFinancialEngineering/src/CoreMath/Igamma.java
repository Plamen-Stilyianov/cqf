...
public class Igamma extends ContFract {
    ...
    private double rgami(double a, double b) {
        System.out.println("rgami called with a==    " + a + "    b==   " + b);
    ...
    public double igamup(double aval, double bval)// R(a,x)if divided by gamma a becomes Q(a,b)   //
    {
        ...
        setInitial(num, den);
        setB(val);
        setFrac(den);// CONTINUED FRACTION DOES //
        evalFract();
        fn = getFrac();
        //leadfactor=leadhilo();
        leadfactor = leadhilo();
    ...
    public final double gcf(double a, double x) {
        ...
        // for (i=1; i<=25; i++)
        {
            ...
            i++;
            //   if (Math.abs(del - 1.0) < 2e-15) break;
        }
        double lead = leadhilo();
        double ret = lead * fn;
        return (ret);
...
/*public final double chisqrc3(double chival,int fval)// Upper tail  for x2<35 v<=30//
{
	double v=fval/2.0;
	double kisqr=chival/2.0;
	double divisor=Function.gamma(v);
	double upgam=gcf(v,kisqr);
//igamratio(v,kisqr);
	double value=upgam/divisor;
	setChisc(value);
	return chival==0? 1 :getChisc();
