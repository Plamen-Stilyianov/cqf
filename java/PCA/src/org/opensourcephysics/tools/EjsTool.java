...
public class EjsTool {
  ...
  static private boolean doRunEjs(String _model, java.util.Set<String> _resources, Class<?> _ejsClass) {
    ...
    try {
      ...
      while (sl != null) {
        if      (sl.startsWith("ejs_root_directory = ")) ejsRootDirPath = sl.substring("ejs_root_directory = ".length()).trim();
        //else if (sl.startsWith("console_options = "))    console_options = sl.substring("console_options = ".length()).trim();
        else if (sl.startsWith("source_directory = "))   sourceDirPath = sl.substring("source_directory = ".length()).trim();
      ...
      if (major<4) { // Incorrect version, update to 4.0
        JOptionPane.showMessageDialog(parentComponent,version+" "+res.getString("EjsTool.IncorrectVersion"),
            res.getString("EjsTool.Error"),JOptionPane.ERROR_MESSAGE);
        return false;
      }
      else { // See if EjsConsole.jar is there
        ejsRootDirectory = new File(ejsRootDirPath);
        if (!new File (ejsRootDirectory,"EjsConsole.jar").exists()) ejsRootDirectory = null;
      }
    } 
    catch (Exception exc) {
      exc.printStackTrace(); 
    ...
    if (ejsRootDirectory==null) {
      // Create a chooser
      JFileChooser chooser = OSPRuntime.createChooser("", new String[]{});
    ...
    if (length>0) for (String res : _resources) extractList.add(res.startsWith(modelPath) ? "./"+res.substring(length) : res);
    else extractList.addAll(_resources);
    java.util.Collections.sort(extractList);
    
    JCheckBox quitCheckBox = new JCheckBox(res.getString("EjsTool.QuitSimulation"),true);
    java.util.List<?> finalList = ejsConfirmList(parentComponent,new java.awt.Dimension(400,400),
      res.getString("EjsTool.ExtractingFiles"),res.getString("EjsTool.Message"),extractList, quitCheckBox);
    ...
    */
    ...
    while (destinationDirectory==null) {
      ...
      if (!destDirPath.startsWith(sourceDirPath)) {
        JOptionPane.showMessageDialog(parentComponent,res.getString("EjsTool.MustBeUnderSource"),
            res.getString("EjsTool.Error"),JOptionPane.ERROR_MESSAGE);
    ...
    for (Iterator<?> it = finalList.iterator(); it.hasNext(); ) {
      ...
      File targetFile = resource.startsWith("./") ? new File (destinationDirectory,resource.substring(2)) : new File(sourceDir,resource);
      ...
      String originalName = resource.startsWith("./") ? modelPath+resource.substring(2) : resource;
      File result = JarTool.extract (originalName,targetFile); // Use the ResourceLoader
      if (result==null) {
        String[] message=new String[]{res.getString("JarTool.FileNotExtracted"),
            originalName+" "+res.getString("JarTool.FileNotExtractedFrom")+" "+_ejsClass.toString()};
    ...
    final String theModel = relativeDir + (_model.startsWith("./") ? _model.substring(2) : _model);
