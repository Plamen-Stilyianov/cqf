 ...
 */
public final class Ibeta extends ContFract {
    ...
    public double betai(double a1, double a2, double xval)// rethink......//
    {
        double d;
        double result;
        double interim;
    ...
    protected void computeFract(int n) // implements the data required to compute the continued fraction   //
    {
        ...
        vars[0] = j == n ? x * i * (b - i) / ((a - 1.0 + j) * (a + j)) : 0.0 - (a + i) * (a + b + i) * x / ((a + j) * (a + 1.0 + j));
        return;// the latest update to be evaluated at the super class  //
    }


    private double betacf(double a, double b, double x) // After Press et al  does the continued fraction only..needs lead for ibeta//
    {
        int n, n2;
